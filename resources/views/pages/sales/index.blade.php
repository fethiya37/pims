...
<section class="content">
    <div class="container-fluid">
            ...
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <div class="row">
                        <div class="col-6">
                            <div class="pl-3">
                                <b>Product Sales: {{ $sales->count() }}</b>
                            </div>
                        </div>
                        <div class="col-6">
                            <button type="button" class="btn btn-primary btn-sm float-right" data-toggle="modal"
                                data-target="#createSaleModal">
                                New Sale
                            </button>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="sale-table" class="table table-bordered table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Sale #</th>
                            ...
                            <tbody>
                                @forelse ($sales as $index => $sale)
                                    <tr>
                                        ...
                                        <td>{{ optional($sale->location)->name ?? 'N/A' }}</td>
                                        ...
                                        <td>
                                            @if ($sale->status == 'pending')
                                                <a type="button" class="btn btn-info btn-sm" data-toggle="modal"
                                                    data-target="#editSaleModal-{{ $sale->id }}">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="{{ route('sales.complete', $sale->id) }}"
                                                    class="btn btn-success btn-sm"
                                                    onclick="return confirm('Complete this sale? Stock will be deducted.');">
                                            ...
                                            <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal"
                                                data-target="#viewSaleModal-{{ $sale->id }}">
        ...
        {{-- CREATE MODAL --}}
        <div class="modal fade" id="createSaleModal">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">New Product Sale</h4>
                        ...
                        <form action="{{ route('sales.store') }}" method="POST">
                            ...
                            <div class="invoice p-3 mb-3">
                                <div class="row">
                                    <div class="col-12">
                                        <h4>
                                            <i class="fas fa-cash-register"></i> Product Sale
                                ...
                                <div class="row invoice-info mb-4">
                                    <div class="col-sm-4">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="vat_rate" class="form-control" step="0.01" min="0"
                                                max="100" value="0">
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
            ...
            @if ($sale->status == 'pending')
                <div class="modal fade" id="editSaleModal-{{ $sale->id }}">
                    <div class="modal-dialog modal-xl">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title">Edit Sale - #{{ $sale->id }}</h4>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                                ...
                                <form action="{{ route('sales.update', $sale->id) }}" method="POST">
                                    ...
                                    <div class="invoice p-3 mb-3">
                                        <div class="row">
                                            <div class="col-12">
                                                <h4>
                                                    <i class="fas fa-cash-register"></i> Edit Sale
                                                    <small class="float-right">Created: {{ \Carbon\Carbon::parse($sale->created_at)->toFormattedDateString() }}</small>
                                                </h4>
                                            </div>
                                        </div>
        ...
        {{-- VIEW MODALS --}}
        @foreach ($sales as $sale)
            <div class="modal fade" id="viewSaleModal-{{ $sale->id }}">
                        ...
                        <div class="modal-body">
                            <div class="invoice p-3 mb-3">
                                <div class="row">
                                    <div class="col-12">
                                        <h4>
                                            <i class="fas fa-cash-register"></i> Sale Record
                                            <small class="float-right">Created: {{ \Carbon\Carbon::parse($sale->created_at)->toFormattedDateString() }}</small>
                                        </h4>
                                    </div>
                                </div>
        ...
        <script type="text/javascript">
            (function() {
                var dataEl = document.getElementById('sale-data');
                ...
                document.querySelector('.add-row')?.addEventListener('click', function(e) {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>
                            <input type="number" name="items[${rowCount}][unit_price]" class="form-control unit-price-input" step="0.01" min="0" required>
                        </td>
                        <td>
                            <div class="d-flex">
                                <button type="button" class="remove-tr btn btn-danger btn-sm mr-1"><b>X</b></button>
                ...
                document.addEventListener('shown.bs.modal', function(e) {
                    var modal = e.target;
                    if (modal.id && modal.id.startsWith('editSaleModal-')) {
                        var saleId = modal.id.replace('editSaleModal-', '');
                        var containerId = 'edit_items_' + saleId;
                ...
                document.addEventListener('click', function(e) {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>
                            <input type="number" name="items[${idx}][unit_price]" class="form-control unit-price-input" step="0.01" min="0" required>
                        </td>
                        <td>
                            <button type="button" class="remove-edit-tr btn btn-danger btn-sm"><b>X</b></button>
                        </td>
                ...
                if (typeof $ !== 'undefined') {
                    ...
                    }).buttons().container().appendTo('#sale-table_wrapper .col-md-6:eq(0)');
                }
            })();
        </script>
    </div>
